...
def generar_pdf(datos, imagen_firma, imagen_foto):
    ...
    try:
        if os.path.exists("logo_campofert.png"):
            ...
            p.drawImage(ImageReader(img_cf), 50, height - 720, width=120, preserveAspectRatio=True, mask='auto')
        if os.path.exists("logo_campolab.png"):
            img_cl = Image.open("logo_campolab.png")
            p.drawImage(ImageReader(img_cl), 440, height - 720, width=120, preserveAspectRatio=True, mask='auto')
    except:
        pass
